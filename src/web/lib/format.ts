// Small pure formatting helpers for the web remote UI. Mirrors the desktop
// renderer's list-utils so the mobile UI reads the same way.
...
export function formatDueDate(dueAtUtc: string | null, dueAt: string | null): DueInfo {
  if (!dueAtUtc) {
    return dueAt ? { text: dueAt, overdue: false } : { text: '', overdue: false };
  }
...
export function humanizeToolName(toolName: string, args?: Record<string, unknown>): string {
  ...
  if (toolName === 'report_intent' && typeof args?.intent === 'string') return args.intent.slice(0, 80);
...
export function describeApproval(approval: { permissionKind: string; intention?: string | null; path?: string | null }): ApprovalDescription {
  ...
  else label = kind.replace(/_/g, ' ') || 'Approval needed';
...
export function agentGlyph(status: string, source?: string): string {
  if (source === 'cca') return '🔀';
