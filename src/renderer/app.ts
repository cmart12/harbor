...
async function openCanvas(spaceId: string, expanded = false): Promise<void> {
  const space = spaces.find(i => i.id === spaceId);
  if (!space) { console.warn('[canvas] openCanvas: space not found', spaceId); return; }

  // In main window, always pop out to separate canvas window
  if (!isCanvasMode) {
    console.log('[canvas] openCanvas: delegating to popout window', { spaceId, title: space.description });
  ...
  console.log('[canvas] openCanvas: loading data for', spaceId);
  canvasView.classList.remove('hidden');
  ...
  console.log('[canvas] readCanvas result:', { error: result.error, contentLen: result.content?.length ?? 0 });

  // Abort if user already switched to another space
  if (canvasMountGen !== myGen) { console.warn('[canvas] openCanvas: aborted (gen mismatch)'); return; }

  if (result.error === 'no_workspace') {
    console.error('[canvas] openCanvas: no_workspace error, canvas will be empty');
    return;
  }

  if (result.error) {
    console.error('[canvas] openCanvas: unexpected error from readCanvas:', result.error);
  }
...
if (isCanvasMode) {
  ...
  whimAPI.onLoadCanvasTarget(async (target: { kind: string; id: string; title: string }) => {
    console.log('[canvas] onLoadCanvasTarget received:', target);
